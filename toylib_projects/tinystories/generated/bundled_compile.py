# ============================================================
# Set up fake devices for JAX (if specified via CLI) before importing JAX and the rest of the code.
# ============================================================
import os
...
setup_fake_devices(num_devices=8)  # Set up 8 fake devices for testing
...
# ============================================================
# toylib_projects.tinystories.train - /Users/anuj/Desktop/code/toylib/toylib_projects/tinystories/train.py
# ============================================================
...
# ============================================================
# None - ../tinystories/scripts/compile.py
# ============================================================
...
Usage (CLI):
    python -m toylib_projects.tinystories.scripts.compile         --num_devices 8         --output_dir /tmp/compile_analysis         --trace_steps 3
...
def _parse_args() -> CompileConfig:
    ...
    parser.add_argument(
        "--batch_size_per_device", type=int, default=4, help="Batch size per device"
    )
    ...
    parser.add_argument(
        "--skip_trace", action="store_true", help="Skip generating Perfetto traces"
    )
    parser.add_argument(
        "--hlo_format",
        type=str,
...
def create_experiment(
    ...
    optimizer_config = create_muon_adam_multi_optimizer_config(
        muon_lr=0.0001,
        adamw_embed_lr=0.0001 * dmodel_lr_scale,
        adamw_output_lr=0.0001 * dmodel_lr_scale,
    )
    dummy_dataset = DummyDataset(
        batch_size=batch_size, seq_len=seq_len, vocab_size=vocab_size
    )
    train_task = Task(name="train", dataset=dummy_dataset)
    eval_task = Task(name="eval", dataset=dummy_dataset)
    ...
    exp = Experiment(
        ...
        checkpoint_config=CheckpointConfig(checkpoint_dir=str(temp_dir)),
        logger_config=LoggerConfig(log_dir=str(temp_dir)),
