...
class _Hdf5FramesSource:
    ...
    def __init__(self, dataset_path: str, label_keys: typing.Optional[tuple[str, ...]]=None) -> None:
    ...
    def __getitem__(self, idx: int):
        self._ensure_open()
        frame = self._f['frames'][idx]
        if self._label_keys is None:
            return frame
        targets = np.array([self._f[f'source/{k}'][idx] for k in self._label_keys], dtype=np.float32)
        return {'frames': frame, 'targets': targets}

    def __getstate__(self) -> dict:
        return {'_path': self._path, '_label_keys': self._label_keys, '_f': None, '_n': self._n, '_height': self._height, '_width': self._width}
...
# ============================================================
# toylib_projects.wm.logger - /Users/anuj/Desktop/code/toylib/toylib_projects/wm/logger.py
# ============================================================
...
# ============================================================
# toylib_projects.wm.metrics - /Users/anuj/Desktop/code/toylib/toylib_projects/wm/metrics.py
# ============================================================
...
# ============================================================
# toylib_projects.wm.experiment - /Users/anuj/Desktop/code/toylib/toylib_projects/wm/experiment.py
# ============================================================
...
class Encoder(Module):
    ...
    def __call__(self, x: jt.Float[jt.Array, 'B H W 3']) -> tuple[jt.Float[jt.Array, 'B h w latent_channels'], jt.Float[jt.Array, 'B h w latent_channels']]:
        h = self.conv_in(x)
        h = self.res1(h)
        h = self.down1(h)
        h = self.res2(h)
...
class Decoder(Module):
    ...
    def __call__(self, z: jt.Float[jt.Array, 'B h w latent_channels']) -> jt.Float[jt.Array, 'B H W 3']:
        h = self.conv_in(z)
        h = self.res1(h)
        h = self.attn(h)
        h = self.res2(h)
...
class VAE(Module):
    ...
    def encode(self, x: jt.Float[jt.Array, 'B H W 3']) -> tuple[jt.Float[jt.Array, 'B h w C'], jt.Float[jt.Array, 'B h w C']]:
        return self.encoder(x)

    def decode(self, z: jt.Float[jt.Array, 'B h w C']) -> jt.Float[jt.Array, 'B H W 3']:
        return self.decoder(z)

    def __call__(self, x: jt.Float[jt.Array, 'B H W 3'], rng_key: typing.Optional[jt.PRNGKeyArray]=None) -> tuple[jt.Float[jt.Array, 'B H W 3'], dict[str, jt.Array]]:
...
def vae_loss(model: VAE, batch: jt.Float[jt.Array, 'B H W 3'], rng_key: jt.PRNGKeyArray, beta: jt.Float[jt.Array, ''] | float=1e-06) -> tuple[jt.Float[jt.Array, ''], dict[str, jt.Array]]:
...
def load_vae(checkpoint_dir: str, step: int, vae_config: ModelConfig, key) -> VAE:
    ...
    """
    template_vae = VAE(config=vae_config, key=key)
    template_vae.init()
    ...
    try:
        restored = manager.restore(step, args=ocp.args.Composite(model=ocp.args.StandardRestore(template)))
    finally:
        manager.close()
...
def create_experiment(train_path: str | Path, val_path: str | Path | None, *, vae_checkpoint_dir: str | None=None, vae_checkpoint_step: int | None=None, base_ch: int=64, latent_channels: int=4, latent_spatial: int=16, hidden_dim: int=256, pooling: Pooling=FLATTEN, batch_size_per_device: int=32, num_microbatches: int=1, max_steps: int=2000, learning_rate: float=0.001, max_grad_norm: float=1.0, eval_interval_steps: int=100, num_eval_steps: int=4, save_interval_steps: int=1000, checkpoint_dir: str='/tmp/wm_probe_ckpt', log_dir: str='/tmp/wm_probe_logs', run_id: str | None=None, wandb_project: str | None=None, wandb_user: str | None=None, seed: int=0, jit_computations: bool=True) -> Experiment:
    ...
    if val_path is not None:
        ...
        eval_task = Task(name='val', dataset=val_ds, metrics=[Loss(), ProbeAuxMetric()])
    optimizer_config = MultiOptimizerConfig(optimizer_configs=[OptimizerConfig(name='frozen', optimizer=optax.set_to_zero()), OptimizerConfig(name='trainable', optimizer=optax.adam(learning_rate=learning_rate))], optimizer_for_param=lambda path: 'frozen' if path[0].name == 'encoder' else 'trainable')
