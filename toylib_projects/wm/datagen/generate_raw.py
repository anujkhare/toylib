...
Usage:
    uv run python toylib_projects/wm/generate_raw.py \\
        --num-episodes 1000 \\
        --output-dir toylib_projects/wm/data/raw \\
...
def main() -> None:
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("toylib_projects/wm/data/raw"),
    )
    ...
    with ShardWriter(args.output_dir, args.episodes_per_shard) as writer:
        for ep_idx in tqdm(range(args.num_episodes), desc="episodes"):
            frames, actions, states = run_episode(env, controller, args.max_steps)
